...
onMounted(async () => {
    ...
    const loaded_account_response = await fetch(`http://127.0.0.1:8000/api/accounts/${Number(props.id)}/loaded`,{
        method: 'GET',
        headers:{
...
<template>
    <section class="section-transactions-for-account">
        <div class="section__inner shell">
            ...
            <div v-if="!is_loading && account && transactions" class="section__card__wrapper">
                <div class="section__card__content">
                    <Card :card="account" :class="{ light: index % 2 == 0, gray: index % 2 == 1 }" /> 
                </div>

                <div v-if="transactions.length > 0" class="section__transactions base-form">
                    <div v-if="transactions" v-for="transaction in transactions" class="section__transaction">
                        ...
                        <div class="transaction__content">
                            ...
                            <div class="content__account__sender">
                                ...
                                <p><span>Вид на сметка:</span> {{ transaction.sender_account.card_type.account_type }}</p>
                                <p><span>Валута: </span> {{ transaction.sender_account.currency.currency }}</p>
                                <p><span>Име на картодържач: </span> {{ transaction.sender_account.user.name }} {{ transaction.receiver_account.user.middle_name ?? '' }} {{ transaction.receiver_account.user.last_name ?? '' }}</p>
                            </div>
                        </div>
                    </div>
                </div>

                <FormErrorMessage v-else text="Няма транзакции по сметката." class="empty__transactions"/>
            </div>
...
.section-transactions-for-account{
    ...
    .section__transaction{
        ...
        gap: 16px;
        padding-block: 12px;
        border-bottom: 1px dashed var(--c-base);
